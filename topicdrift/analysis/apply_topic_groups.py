...
def write_registry(
    ...
    print(
        f"  wrote {prefix}topic_groups.parquet + {prefix}topic_groups.csv "
        f"({len(reg)} groups)"
    )
    return reg
...
def render_md(
    ...
    group_total = {
        g["name"]: sum(size_by_label.get(t, 0) for t in g["topics"]) for g in groups
    }
    ...
    lines.append("")
    lines.append(
        "Generated by src/analysis/apply_topic_groups.py — do not edit by hand."
    )
    lines.append("")
...
def main(
    ...
    topics_out = topics_out.drop(
        columns=[c for c in ("group_color",) if c in topics_out.columns]
    )
    ...
    if tot_path.exists():
        ...
        print(
            f"  stamped group into {prefix}topics_over_time.parquet ({len(tot)} rows)"
        )
    ...
    print(
        f"  regenerated {prefix}topic_groups.md ({len(order)} groups, {total:,} papers)"
    )
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--prefix", default="conf_", help="data file prefix, e.g. conf_ or icse_"
    )
    ap.add_argument(
        "--config", default=str(CONFIG_PATH), help="grouping YAML (source of truth)"
    )
