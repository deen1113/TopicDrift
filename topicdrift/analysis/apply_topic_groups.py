...
def build_label_map(groups: list[dict]) -> tuple[dict[str, str], dict[str, str], list[str]]:
    ...
    for g in groups:
        ...
        for label in g.get("topics", []):
            ...
            if label in label_to_group:
                raise ValueError(
                    f"label {label!r} listed twice "
                    f"({label_to_group[label]!r} and {name!r})"
                )
            label_to_group[label] = name
    return label_to_group, group_color, order


def validate(label_to_group: dict[str, str], topic_labels: set[str],
             config_name: str = "the grouping YAML") -> None:
...
def write_registry(order: list[str], group_color: dict[str, str],
                   group_papers: dict[str, int], total: int,
    ...
    print(f"  wrote {prefix}topic_groups.parquet + {prefix}topic_groups.csv "
          f"({len(reg)} groups)")
    return reg


def render_md(groups: list[dict], topics: pd.DataFrame, total: int,
              title: str, config_name: str) -> str:
    ...
    group_total = {
        g["name"]: sum(size_by_label.get(t, 0) for t in g["topics"])
        for g in groups
    }
    ...
    for i, g in enumerate(ordered, 1):
        ...
        lines.append("|---|---:|---:|")
        members = sorted(
            g["topics"], key=lambda t: size_by_label.get(t, 0), reverse=True
        )
        for t in members:
...
def main(prefix: str = "conf_", config_path: Path = CONFIG_PATH,
         title: str = "All Conferences") -> None:
    ...
    id_to_group = {
        int(r["topic_id"]): label_to_group[str(r["llm_label"])]
    ...
    print(f"  stamped group into {prefix}topics.parquet "
    ...
    group_papers = {
        g: int(fitted[fitted["topic_id"].map(id_to_group) == g]["size"].sum())
        for g in order
    }
...
if __name__ == "__main__":
    ...
    ap.add_argument("--prefix", default="conf_",
                    help="data file prefix, e.g. conf_ or icse_")
    ap.add_argument("--config", default=str(CONFIG_PATH),
                    help="grouping YAML (source of truth)")
